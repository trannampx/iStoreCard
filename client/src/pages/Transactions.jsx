...
function Transactions() {
  ...
  const qrScannerRef = useRef(null); // Ref để giữ instance scanner, tránh memory leak
  ...
  const handleQRImageUpload = async (e) => {
    ...
    // Nén ảnh trước khi quét để tránh tràn RAM
    const compressAndScan = () => {
      ...
      reader.onload = (event) => {
        ...
        img.onload = () => {
          ...
          canvas.toBlob((blob) => {
            processFile(blob ? new File([blob], file.name, { type: 'image/jpeg' }) : file);
          }, 'image/jpeg', 0.9);
        };
  ...
  }, [isLiveScannerOpen]); // eslint-disable-line react-hooks/exhaustive-deps
